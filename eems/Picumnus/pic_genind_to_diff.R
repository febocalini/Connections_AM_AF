...
my_vcf_teste <- read.vcfR("pic_incomp_z_1SNP.vcf")
#3839 snps
genind_pic <- vcfR2genind(my_vcf_teste)
class(genind_pic)

nInd(genind_pic)
#16
#genotype matrix
Geno2 <- genind_pic@tab
dim(Geno2)
## Keep SNPs that are observed to be bi-allelic.
multi.loci <- names(which(genind_pic@loc.n.all != 2))
...
stopifnot(identical(genind_pic@type, 'codom'))
...
#version1
write.table(Diffs_v1, "pic_eems.diffs", 
            col.names = FALSE, row.names = FALSE, quote = FALSE)
#version2
write.table(Diffs_v2, "pic_eems2.diffs", 
            col.names = FALSE, row.names = FALSE, quote = FALSE)

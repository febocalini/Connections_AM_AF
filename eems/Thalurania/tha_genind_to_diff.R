...
my_vcf_teste <- read.vcfR("tha_incomp_snapp_1SNP.vcf")
#4014 snps
genind_tha <- vcfR2genind(my_vcf_teste)
class(genind_tha)

nInd(genind_tha)
#50
ind_names <- indNames(genind_tha)
write.csv(ind_names, file="ind_names_tha.csv")

############ sem ridgway e 95813 #######################

genind_tha <- genind_tha[indNames(genind_tha) != "Tha_fur_FMNH393976", drop=T,]
genind_tha <- genind_tha[indNames(genind_tha) != "Tha_fur_LSU55204", drop=T,]

########### removendo fannyie e colombica ##########################

genind_tha <- genind_tha[indNames(genind_tha) != "Tha_col_LSU11709", drop=T,]
genind_tha <- genind_tha[indNames(genind_tha) != "Tha_fan_LSU52956", drop=T,]
genind_tha <- genind_tha[indNames(genind_tha) != "Tha_fan_LSU7821", drop=T,]

ind_names <- indNames(genind_tha)
write.csv(ind_names, file="ind_names_tha.csv")
#genotype matrix
Geno2 <- genind_tha@tab
dim(Geno2)
## Keep SNPs that are observed to be bi-allelic.
multi.loci <- names(which(genind_tha@loc.n.all != 2))
...
stopifnot(identical(genind_tha@type, 'codom'))

#Since the labeling does not matter for computing differences, I thak the second allele to label as the "derived" allele. 
#That is, I pick all loci whose name ends with .01.
#Geno3 <- Geno2[, str_detect(colnames(Geno2), "\\.01$")] ### essa linha acaba com tudo, não rodar
...
#version1
write.table(Diffs_v1, "tha_eems.diffs", 
            col.names = FALSE, row.names = FALSE, quote = FALSE)
#version2
write.table(Diffs_v2, "tha_eems2.diffs", 
            col.names = FALSE, row.names = FALSE, quote = FALSE)

...
###### Final ModleR picothraustes ######
setwd("/home/fernanda/lgm_all_sp_sdm_artigo2")

###### dados para picothraustes lgm ######

occu_pic <- read.csv("Picumnus_exilis_occ.csv", header=T)
...
sdmdata_1sp <- setup_sdmdata(species_name = "lgm_pic",
...
many <- do_many(species_name = "lgm_pic",
                predictors = preds_lgm,
                models_dir = final_folder,
                png_partitions = TRUE,
                write_bin_cut = FALSE,
...
final_model(species_name = "lgm_pic",
...
ens <- ensemble_model(species_name = "lgm_pic",
                      occurrences = occu_pic,
...
save.image('lgm_pic_final_sdm_results.RData')

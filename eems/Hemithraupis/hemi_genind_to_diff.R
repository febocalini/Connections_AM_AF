...
my_vcf_teste <- read.vcfR("hemi_incomp_z_1SNP.vcf")
#3902 snps
genind_hemi <- vcfR2genind(my_vcf_teste)
class(genind_hemi)

nInd(genind_hemi)
#13
#genotype matrix
Geno2 <- genind_hemi@tab
dim(Geno2)
## Keep SNPs that are observed to be bi-allelic.
multi.loci <- names(which(genind_hemi@loc.n.all != 2))
...
stopifnot(identical(genind_hemi@type, 'codom'))
...
#version1
write.table(Diffs_v1, "hemi_eems.diffs", 
            col.names = FALSE, row.names = FALSE, quote = FALSE)
#version2
write.table(Diffs_v2, "hemi_eems2.diffs", 
            col.names = FALSE, row.names = FALSE, quote = FALSE)
